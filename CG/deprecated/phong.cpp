...
int selectMode = 1;
GLuint phongProg;   // handles to objects
...
void display(void)
{
    ...
    switch(selectMode)
    {
        case USE_PHONG_SHADING:
            glUseProgram(shader->getProgram());//phongProg);
        break;
...
void init(void)
{
    ...
    shader = new glcShader();
    shader->setShaders("../shaders/phong.vert", "../shaders/phong.frag");

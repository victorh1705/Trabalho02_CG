...
void glcWavefrontObject::Draw(GLuint mode)
{
    static GLuint i;
    ...
    /* do a bit of warning */
    if (mode & FLAT_SHADING && !model->facetnorms)
    {
        printf("glmDraw() warning: flat render mode requested "
               "with no facet normals defined.\n");
        mode &= ~FLAT_SHADING;
    }
    if (mode & SMOOTH_SHADING && !model->normals)
    {
        printf("glmDraw() warning: smooth render mode requested "
               "with no normals defined.\n");
        mode &= ~SMOOTH_SHADING;
    }
    if (mode & USE_TEXTURE && !model->texcoords)
    {
        printf("glmDraw() warning: texture render mode requested "
               "with no texture coordinates defined.\n");
        mode &= ~USE_TEXTURE;
    }
    if (mode & FLAT_SHADING && mode & SMOOTH_SHADING)
    {
        printf("glmDraw() warning: flat render mode requested "
               "and smooth render mode requested (using smooth).\n");
        mode &= ~FLAT_SHADING;
    }
    if (mode & USE_COLOR && !model->materials)
    {
        printf("glmDraw() warning: color render mode requested "
               "with no materials defined.\n");
        mode &= ~USE_COLOR;
    }
    if (mode & USE_MATERIAL && !model->materials)
    {
        printf("glmDraw() warning: material render mode requested "
               "with no materials defined.\n");
        mode &= ~USE_MATERIAL;
    }
    if (mode & USE_COLOR && mode & USE_MATERIAL)
    {
